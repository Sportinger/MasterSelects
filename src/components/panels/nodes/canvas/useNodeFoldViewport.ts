...
export function useNodeFoldViewport(canvas: RefObject<HTMLDivElement | null>, source: NodeGraph, target: NodeGraph,
  ...
  const pending = useRef<{ graph: NodeGraph; collapsed: boolean; from: Viewport; started: number } | null>(null);
  ...
  useLayoutEffect(() => {
    ...
    const fit = fittedNodeViewport(bounds, element.clientWidth, element.clientHeight);
