...
export interface OperatorParameter {
  id: string; label: string; type: 'number' | 'boolean' | 'vector' | 'select' | 'color' | 'text'; default: OperatorValue;
  /** Maximum character count for text parameters. */
  maxLength?: number;
...
export interface BoundOperatorNode {
  ...
  /** Value published as a keyframeable Effects tab parameter through `bindings.value`. */
  exposed?: { label: string; min?: number; max?: number; step?: number };
