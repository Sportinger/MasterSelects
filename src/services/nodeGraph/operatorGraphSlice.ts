...
export function selectOperatorGraphSlice(graph: EffectOperatorGraph, args: Record<string, unknown>) {
  ...
  if (!Number.isSafeInteger(hops) || Number(hops) < 0) throw new Error('hops must be a non-negative safe integer.');
  if (!['upstream', 'downstream', 'both'].includes(String(direction))) throw new Error('Invalid neighbor direction.');
  const ids = args.nodeIds;
  if (ids !== undefined && (!Array.isArray(ids) || !ids.length || ids.some(id => typeof id !== 'string') || new Set(ids).size !== ids.length)) throw new Error('nodeIds must contain distinct node IDs (at least one).');
  ...
  for (let hop = 0; hop < Number(hops); hop++) {
    ...
    if (selected.size === previous.size) break;
  }
