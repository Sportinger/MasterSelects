...
const COLORS: Record<string, string> = {
  image: '#78bded', texture: '#78bded', depth: '#a9a9f8', geometry: '#e1ac72', mesh: '#e1ac72', landmarks: '#e5a0c8',
...
const GENERIC_FORMATS: Record<string, string[]> = {
  texture: ['2D image / color texture'], geometry: ['Geometry supplied by the connected clip or node'], mesh: ['Vertices + triangle indices'],
...
export function describeNodePort(port: { type: string; metadata?: NodeGraphPortMetadata }): NodePortContract & { color: string; formatLabels: string[] } {
  ...
  const fallback: NodePortContract = {
    ...
    description: flockType ? 'Flock signal. Connect to the same Flock signal type.' : 'Intermediate graph signal. The owning node validates supported data and connections.',
