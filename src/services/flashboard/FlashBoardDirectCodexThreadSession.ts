...
export function buildDirectCodexBaseInstructions(): string {
  return [
    ...
    'Image results (captureFrame, preview frames) arrive as content items: the text item plus { type: "inputImage", imageUrl }. Inside exec, pass the inputImage item to image(item.imageUrl, "low"); never text() or JSON.stringify a result that contains an image.',
