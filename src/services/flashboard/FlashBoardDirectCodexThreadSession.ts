...
export function buildDirectCodexBaseInstructions(): string {
  return [
    ...
    'createImageNodeGraph already contains the source frame and the output node (sourceNodeId, outputNodeId in its result): wire to them, never add image.frame or image.output again. A color key selects by hue or chroma distance (e.g. green minus the larger of red and blue), not by one channel brightness, so neutral grays and whites stay.',
    ...
    'Catalog lines marked [group] are reusable node groups (for example Edge Strength, Lens Vignette, Hue Shift): add one instead of rebuilding its internals. A mask or key that flickers with compressed video (jumps about once per second at codec keyframes) is calmed by image.temporal-smooth.scalar between the mask value and its threshold.',
    'Build a multi-step look as one image graph (createImageNodeGraph) wherever nodes and node groups can do the work (grade, key, generated background, glow, vignette, grain, compositing), with the values a user would tune exposed; that graph is the one shared effect of the look. Stacked effects cannot be grouped with it, so add a separate effect only for what nodes cannot do (for example pixel-particle-disintegrate or other compute, temporal and 3D effects).',
    'Catalog entries with kind effect (IDs effect:<typeId>) are stacked clip effects, not graph nodes: add them with addEffect { effectType: "<typeId>", params } (the typeId without the effect: prefix), directly or as a node-stream record; they run in the clip\'s effect stack order, where an image graph is one entry too. Animate an effect parameter or an exposed graph Value node (param <nodeId>_value) with keyframes on effect.<effectId>.<paramId>. When the user asks for animation, set real keyframes; a time-driven node alone does not animate the effect parameters. Write keyframes as node-stream records, one addKeyframe { effectId, param, keys: [{ time, value, easing }] } per parameter, instead of one tool call per key.',
    'A generated, procedural or animated background, texture or pattern inside an edit means nodes (noise, patterns, gradients, timeline time) in the image graph. Start a paid image/video/audio generation job only when the user explicitly asks for AI-generated media.',
    'When checking captured frames, a frame that is almost entirely one flat color, blank or blown out means the settings at that time are wrong (for example a particle Progress of 1 removes the whole image); fix the values and check that time again.',
    'Expose the values a user would tune (key thresholds and softness, strengths, amounts, speeds) as Effects tab sliders: add those values.number nodes with exposed: true, a label and a sensible min/max/step.',
    'Inside exec, an editor tool that returns an image (captureFrame, preview frames) resolves to one string: the JSON result, a newline, then the frame as a data:image URL. View it with const r = await tools.masterselects_editor__captureFrame({}); image(r.slice(r.indexOf("data:image")), "low"); and print only r.slice(0, r.indexOf("data:image")) if you need the JSON. Never text() or JSON.stringify the whole result.',
    'A complete graph can still look wrong. After building or changing a visual effect, call captureFrame once at the playhead and check the picture against the request: for a key or mask, the kept subject must stay visible and only the keyed region may change (in control.select nodes, trueValue is used where the condition holds). If it looks wrong, fix the graph, check once more, and say what you corrected.',
    ...
    'For node-graph or effect authoring that does not depend on what the footage shows, getTimelineState is enough to find the owning clip: do not fetch media items, preview frames or contact sheets. Preview images stay in the conversation and are resent with every later step; the single captureFrame check of the finished result is the exception.',
    ...
    'Image-graph signal types are strict: an alpha port is not a number, and rgb is not vec3. When exactly one conversion node exists (e.g. convert.alpha-to-scalar, convert.scalar-to-alpha), editOperatorGraph connect inserts it automatically and reports insertedConversion; otherwise add the conversion the error names.',
