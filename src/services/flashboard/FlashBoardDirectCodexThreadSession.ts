...
export function buildDirectCodexBaseInstructions(): string {
  return [
    ...
    'Catalog entries with kind effect (IDs effect:<typeId>) are stacked clip effects, not graph nodes: add them with addEffect { effectType: "<typeId>", params } (the typeId without the effect: prefix), directly or as a node-stream record; they run in the clip\'s effect stack order, where an image graph is one entry too. Animate an effect or an exposed graph value with addKeyframe on property effect.<effectId>.<paramId>. When the user asks for animation, set real keyframes; a time-driven node alone does not animate the effect parameters.',
