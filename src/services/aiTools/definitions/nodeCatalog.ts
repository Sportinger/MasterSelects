...
export const nodeCatalogToolDefinitions: ToolDefinition[] = [
  ...
  { type: 'function', function: {
    ...
    description: 'Search registered nodes and visual/audio effects by ID, name, description or signal type. The complete compact base inventory is already in editorNodeCatalog at turn start. Returns bounded summaries; getNodeDefinitions reads exact contracts for selected IDs. Availability depends on the graph owner; signal-type filters do not validate a connection.',
    parameters: { type: 'object', additionalProperties: false, properties: {
      ...
      limit: { type: 'integer', minimum: 1, maximum: 30, description: 'Maximum results. Default 12.' },
    }, required: [] },
  } },
  { type: 'function', function: {
    name: 'getNodeDefinitions',
    description: 'Read public authoring contracts for up to eight exact node/effect catalog IDs: underlying type IDs, owner contexts, ports, parameter defaults, bounds and choices. Missing IDs are reported. Does not create nodes or establish connection compatibility. Built-in clip stages have source-dependent contracts.',
    parameters: { type: 'object', additionalProperties: false, properties: {
