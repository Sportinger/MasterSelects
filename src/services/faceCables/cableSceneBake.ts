...
export function createCableSceneBake(cables: FaceCableConfig[], fps: number, frames: number, duration: number, aspect: number, depthGrid?: CableDepthGrid, depthBinding?: string) {
  ...
  const scene: CableSceneBake = { version: depthGrid ? 2 : 1, depthGrid, depthBinding, cables, fps, frames, duration, triangles, outline, data };
  return {
    scene,
    reuseDepth(frame: number, previous: CableSceneBake) {
      if (!depthGrid || !previous.depthGrid || previous.frames !== frames || previous.fps !== fps || previous.duration !== duration
        ...
        || (previous.depthBinding !== undefined && previous.depthBinding !== depthBinding)) throw new Error('Saved depth no longer matches this clip. Use Bake cables to estimate fresh depth.');
