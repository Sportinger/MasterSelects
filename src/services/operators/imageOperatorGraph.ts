...
function compileImageOperatorTarget(graph: EffectOperatorGraph, params: Record<string, unknown>, preview?: ImageOperatorPreviewTarget): ImageOperatorPlan {
  ...
  const emit = (instruction: ImagePlanInstruction) => instructions.push(instruction) - 1;
  ...
  function visit(current: BoundOperatorNode, output: string): number {
    const cacheKey = `${current.id}:${output}`;
    const cached = registers.get(cacheKey); if (cached !== undefined) return cached;
    if (visiting.has(current.id)) throw new Error('Image graph contains a cycle.');
    ...
    switch (current.operator) {
      ...
      case 'values.number': {
        ...
        register = emit({ nodeId: current.id, operation: 'constant', type: 'scalar', inputs: [], value }); break;
      }
    ...
    visiting.delete(current.id); registers.set(cacheKey, register); return register;
  }
  ...
  const parameters = capabilities.includes('uv') ? 'inputColor: vec4f, inputUv: vec2f' : 'inputColor: vec4f';
  return { fusion: 'inline', capabilities, instructions, output, key: `image-v1-${hash(canonical)}`,
    wgsl: [IMAGE_COLOR_WGSL, `fn evaluateImageGraph(${parameters}) -> vec4f {`, `  let pixel = inputColor;`, ...expressions, `  return ${returned};`, `}`].join('\n') };
}
