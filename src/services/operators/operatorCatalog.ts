...
export function listNodeCatalog(): NodeCatalogEntry[] {
  ...
  const operators: NodeCatalogEntry[] = EFFECT_OPERATORS.filter(o => !legacy.has(o.id)).map(o => ({
    ...
    context: SCENE_OPERATORS.includes(o) ? `3D image surfaces${isVoxelOperator(o.id) ? ' + Voxel Relief' : ''}`
      : isVoxelOperator(o.id) ? 'Voxel Relief' : o.id === 'forces.wind' ? 'Face Cables + Flock' : 'Face Cables',
  }));
