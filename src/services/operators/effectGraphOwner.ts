...
export function hasEffectOperatorGraph(type: string): boolean { return type === 'face-cables' || type === 'voxel-relief' || type === 'invert' || type === 'analog-signal-lab'; }
...
export function effectOperatorGraph(effect: EffectGraphOwner): EffectOperatorGraph {
  ...
  if (effect.type === 'invert') {
    const saved = effect.operatorGraph ?? readEffectGraph(effect.params[EFFECT_GRAPH_PARAM], createDefaultInvertImageGraph);
    const graph = migrateImageOperatorGraph(saved);
    const errors = validateEffectGraph(graph, typeof graph.incomplete === 'string');
    if (errors.length) throw new Error(errors[0]);
    if (!graph.incomplete) compileImageOperatorGraph(graph, effect.params);
    return graph;
  }
...
export function validateEffectOwnerGraph(effect: Pick<Effect, 'type'>, graph: EffectOperatorGraph, params: Record<string, unknown>) {
  ...
  else if (effect.type === 'invert') compileImageOperatorGraph(graph, params as Record<string, never>);
