...
export const EFFECT_OPERATORS: readonly OperatorDefinition[] = [
  ...
  stage('tracking.face', 'Face tracking', [port('image', 'image', true)], [port('landmarks', 'landmarks')], [], 'analysis'),
  stage('tracking.anchors', 'Face anchors', [port('landmarks', 'landmarks', true)], [port('anchors', 'anchors')]),
  { ...stage('depth.estimate', 'Depth estimation', [port('image', 'image', true)], [port('depth', 'depth')], [{ ...number('strength', 'Depth strength', 1, 0.1, 2), animatable: false }], 'analysis'), bypass: 'mute', runtime: 'worker' },
  ...
  stage('render.cables', 'Cable rendering', [port('curves', 'curves', true), port('surface', 'surface')], [port('scene', 'scene')], [
