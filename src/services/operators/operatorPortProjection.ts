...
export function projectOperatorPort(p: OperatorPort, direction: 'input' | 'output'): NodeGraphPort {
  const type: NodeGraphSignalType = p.type === 'image' || p.type === 'depth' || p.type === 'texture' ? 'texture' : p.type === 'number' || p.type === 'field' ? 'number' : p.type === 'boolean' ? 'boolean' : ['scene', 'camera', 'light'].includes(p.type) ? 'scene' : 'geometry';
