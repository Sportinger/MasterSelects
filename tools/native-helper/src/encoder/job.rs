...
/// Find the FFmpeg CLI binary, preferring bundled version next to our executable
fn find_ffmpeg_binary() -> PathBuf {
    // Check for bundled ffmpeg next to our binary
    if let Ok(exe_path) = std::env::current_exe() {
        if let Some(exe_dir) = exe_path.parent() {
            let bundled = exe_dir.join(ffmpeg_exe_name());
            if bundled.exists() {
                debug!("Using bundled FFmpeg: {}", bundled.display());
                return bundled;
            }

            // Also check ffmpeg subdirectory (e.g. ffmpeg/win64/bin/ffmpeg.exe)
            let subdir = exe_dir.join("ffmpeg").join("bin").join(ffmpeg_exe_name());
            if subdir.exists() {
                debug!("Using FFmpeg from subdirectory: {}", subdir.display());
                return subdir;
            }
        }
    }

    // Check FFMPEG_DIR environment variable
    if let Ok(ffmpeg_dir) = std::env::var("FFMPEG_DIR") {
        let from_env = PathBuf::from(&ffmpeg_dir).join("bin").join(ffmpeg_exe_name());
        if from_env.exists() {
            debug!("Using FFmpeg from FFMPEG_DIR: {}", from_env.display());
            return from_env;
        }
    }
...
impl EncodeJob {
    ...
    pub fn start(&mut self) -> Result<EncodeJobHandle> {
        ...
        info!(
            "Starting encode job: {}x{} @ {} fps, {} frames, codec: {:?}, ffmpeg: {}",
            self.output.width, self.output.height, self.output.fps, self.frame_count,
            self.output.codec, ffmpeg_path.display()
        );
    ...
    fn build_video_args(&self) -> Vec<String> {
        match self.output.codec {
            VideoCodec::Prores => {
                let profile = self.output.profile.as_deref().unwrap_or("3");
