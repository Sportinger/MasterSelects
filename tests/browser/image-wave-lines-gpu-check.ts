...
import { colorToRgba, normalizeCatalogColor } from '../../src/effects/_shared/catalogColor';
import { imageGraphProgramShader } from '../../src/effects/_shared/imageGraphDefinition';
import { waveLines } from '../../src/effects/analog';
...
const width = 63, height = 38, bytesPerRow = 256;
const cases = [
  ...
  { name: 'timeline-colors', params: { scale: 17, amount: .83, speed: 3.25, colorA: '2a70d6c0', colorB: '#e13b4d40' }, time: 1.375 },
] as const;

function resolvedParams(overrides: Record<string, number | string>): Record<string, unknown> {
  const result = { ...Object.fromEntries(Object.entries(waveLines.params).map(([id, spec]) => [id, spec.default])), ...overrides };
  for (const id of ['colorA', 'colorB'] as const) {
    const fallback = waveLines.params[id].default;
    if (typeof fallback !== 'string') throw new Error(`Wave Lines parameter ${id} has no color default`);
    result[id] = normalizeCatalogColor(result[id] as number | string | undefined, fallback);
  }
  return result;
}
...
function numeric(params: Record<string, unknown>, id: string): number {
  ...
  if (typeof value !== 'number') throw new Error(`Wave Lines parameter ${id} is not numeric`);
  return value;
}

function packLegacy(params: Record<string, unknown>, time: number): Float32Array {
  const colorA = colorToRgba(params.colorA as string, waveLines.params.colorA.default as string);
  const colorB = colorToRgba(params.colorB as string, waveLines.params.colorB.default as string);
...
function fixture(): Uint8Array {
  ...
  for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) result.set([
    (x * 31 + y * 11) & 255, (x * 5 + y * 43) & 255,
    (x * 19 + y * 17) & 255, (x * 47 + y * 67) & 255,
  ], y * bytesPerRow + x * 4);
  return result;
}
...
function directGraph(): EffectOperatorGraph {
  const graph = structuredClone(createDefaultWaveLinesGraph());
  const frame = graph.nodes.find(node => node.operator === 'image.frame');
  const output = graph.nodes.find(node => node.operator === 'image.output');
  if (!frame || !output) throw new Error('Wave Lines default graph lacks image boundaries');
  graph.edges = graph.edges.filter(edge => edge.to !== output.id);
  graph.edges.push({ id: 'wave-lines-test-direct-output', from: frame.id, output: 'image', to: output.id, input: 'image' });
  return graph;
}

export async function checkWaveLinesGraphGpu(device: GPUDevice): Promise<number> {
  const input = fixture();
  ...
  try {
    for (const item of cases) {
      ...
      const expected = await render(device, sampler, source.createView(), target, readback,
        waveLines.shader, waveLines.entryPoint, packLegacy(params, item.time));
      const actual = await render(device, sampler, source.createView(), target, readback,
        imageGraphProgramShader(plan, 'waveLinesGraphFragment'), 'waveLinesGraphFragment',
    ...
    const direct = await render(device, sampler, source.createView(), target, readback,
      imageGraphProgramShader(plan, 'waveLinesDirectFragment'), 'waveLinesDirectFragment',
    ...
    if (!processed || processed.every((value, index) => value === direct[index])) throw new Error('rewired Wave Lines branch did not change processed output');
