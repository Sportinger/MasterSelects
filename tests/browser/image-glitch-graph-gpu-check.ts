...
import { glitch } from '../../src/effects/analog';
...
const cases = [
  ...
  { name: 'timeline-tick', params: { scale: 9.5, amount: .83, speed: 3.25 }, time: 1.375 },
] as const;

function params(overrides: Record<string, number>): Record<string, unknown> {
  return { ...Object.fromEntries(Object.entries(glitch.params).map(([id, spec]) => [id, spec.default])), ...overrides };
}
function numberAt(values: Record<string, unknown>, id: string) {
  const value = values[id];
  if (typeof value !== 'number') throw new Error(`Glitch parameter ${id} is not numeric`);
  return value;
}
...
function directGraph(): EffectOperatorGraph {
  const graph = structuredClone(createDefaultGlitchGraph());
  const frame = graph.nodes.find(node => node.operator === 'image.frame'), output = graph.nodes.find(node => node.operator === 'image.output');
  if (!frame || !output) throw new Error('Glitch default graph lacks image boundaries');
  graph.edges = graph.edges.filter(edge => edge.to !== output.id);
  graph.edges.push({ id: 'glitch-test-direct-output', from: frame.id, output: 'image', to: output.id, input: 'image' });
  return graph;
}

export async function checkGlitchGraphGpu(device: GPUDevice): Promise<number> {
  ...
  try {
    for (const item of cases) {
      const values = params(item.params), plan = compileImageOperatorGraph(createDefaultGlitchGraph(), values);
      const expected = await render(device, sampler, source.createView(), target, readback, glitch.shader, glitch.entryPoint, legacyUniforms(values, item.time));
      const actual = await render(device, sampler, source.createView(), target, readback, imageGraphProgramShader(plan, 'glitchGraphFragment'),
        'glitchGraphFragment', packImageOperatorRuntimeUniforms(plan, item.time, width, height));
    ...
    const plan = compileImageOperatorGraph(directGraph(), params({ amount: 1, scale: 14, speed: 1 }));
    const direct = await render(device, sampler, source.createView(), target, readback, imageGraphProgramShader(plan, 'glitchDirectFragment'),
      'glitchDirectFragment', packImageOperatorRuntimeUniforms(plan, 0, width, height));
    ...
    if (!processed || processed.every((value, index) => value === direct[index])) throw new Error('rewired Glitch branch did not change processed output');
