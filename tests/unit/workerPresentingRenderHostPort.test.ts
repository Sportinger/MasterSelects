...
describe('worker presenting render host port', () => {
  ...
  it('passes visible video layer blend, opacity, and ordered effect stacks into worker GPU WebCodecs streams', async () => {
    const fallback = createFallback();
    const bridge = createBridge();
    const offscreen = { width: 640, height: 360 } as unknown as OffscreenCanvas;
    installWorkerCanvasSupport(offscreen);
      ...
      expect.objectContaining({
        ...
        layers: [
          ...
          expect.objectContaining({
            ...
            opacity: 0.5,
            blendMode: 'screen',
            // Chroma Key now uses the canonical graph path. The complete stack
            // travels in renderLayer so the worker preserves its original order;
            // inline effects stay neutral to avoid applying them twice.
            inlineBrightness: 0,
            inlineContrast: 1,
            inlineSaturation: 1,
            inlineInvert: false,
            chromaKeyMode: 0,
            levelsEnabled: false,
            complexEffectCount: topLayer.effects.length,
            renderLayer: expect.objectContaining({
              id: topLayer.id,
              effects: topLayer.effects,
            }),
          }),
        ],
      }),
