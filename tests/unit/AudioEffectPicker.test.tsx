...
describe('audio effect tile catalog', () => {
  it('filters by search and category and adds from a text tile', () => {
    ...
    expect(screen.queryByRole('button', { name: 'Audio Math', exact: true })).toBeNull();
    fireEvent.change(screen.getByLabelText('Search effects'), { target: { value: 'delay' } });
    expect(screen.queryByRole('button', { name: 'Graphic EQ', exact: true })).toBeNull();
    ...
    fireEvent.click(screen.getByRole('option', { name: 'Time & Space', exact: true }));
    expect(screen.queryByRole('button', { name: 'Graphic EQ', exact: true })).toBeNull();
