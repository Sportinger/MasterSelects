...
describe('DOM node values', () => {
  ...
  it.each([false, true])('keeps real editing working with canvas rendering %s', async canvasRendered => {
    ...
    const view = render(<NodeValuePreview node={node} canvasRendered={canvasRendered} />);
    ...
    expect(value.closest('.node-value-canvas-control') !== null).toBe(canvasRendered);
    if (canvasRendered) {
      expect(value).toHaveAttribute('tabindex', '0');
      fireEvent.keyDown(value, { key: 'Enter' });
    } else fireEvent.doubleClick(value);
    ...
    if (canvasRendered) expect(view.container.querySelector('output')).toBeNull();
    else expect(screen.getByRole('status', { name: 'Multiply output value live value' })).toHaveTextContent('—');
  });
});
