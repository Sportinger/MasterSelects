...
vi.mock('../../src/stores/mediaStore', () => ({ useMediaStore: { getState: () => ({ files: [{ id: 'media', type: 'video', width: 100, height: 100 }] }) } }));
...
it('keeps Slit Scan active after a track edit removes the reference and resumes correction when restored', () => {
  ...
  try { expect(render).toThrow(/reference-gap/); } finally { finish(); }
