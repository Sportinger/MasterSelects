...
| **Color & effects** | Grade through Color Nodes or the synchronized Color controls, inspect curves and scopes, combine GPU effects and transitions, and animate masks and properties with keyframes. Invert, Brightness, Contrast, and Saturation expose executable groups of shared vector and math nodes, preserving alpha and using the existing render passes. Analog Signal Lab exposes its PAL, RF, VHS, receiver, decoder, and CRT processing as an editable compute graph. |
