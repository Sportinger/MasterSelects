...
| **Color & effects** | Grade through Color Nodes or the synchronized Color controls, inspect curves and scopes, combine GPU effects and transitions, and animate masks and properties with keyframes. All nine color effects plus Threshold and Posterize expose executable groups of shared vector, math, and color-conversion nodes, preserving alpha and using the existing render paths. Analog Signal Lab exposes its PAL, RF, VHS, receiver, decoder, and CRT processing as an editable compute graph. |
