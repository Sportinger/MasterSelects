...
| **3D** | Combine footage with models, lights, cameras, and Gaussian splats in a shared scene. Large splats parse off the UI thread with visible progress. Splat Exploration exposes editable nodes for stretched splats, particle simulation, camera fading, and an approximate mesh wireframe, with explicit per-branch budgets. |
