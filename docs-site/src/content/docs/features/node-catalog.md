...
Codex Direct and Fast do not carry the node inventory in every conversation. The first prompt of a new thread only states how to fetch it: `searchNodeCatalog({ list: true })` returns every addable registered operator, visual effect, Flock node, audio effect, parameter source, Color node and common field-backed clip stage as a plain-text list, one line each (exact catalog ID, input/output signal types, short purpose), grouped by owning context. Fixed graph anchors are marked; internal-only stages are omitted. Turns without node work therefore skip its ~8k tokens on every model step. Project-local custom definitions are outside this base inventory. (The kernel Fast V2 request keeps its JSON `editorNodeCatalog` contract.)

Two read-only tools provide more detail:

`getNodeDefinitions` returns compact definitions by default: ports as `id:type` (`!` required, `*` repeated) and parameters as `id:type [min..max] {options} =default`, with a `legend` field; `detail: "full"` returns the complete JSON contracts. The Direct instructions ask the agent to request every needed ID in one call, because each extra model step rereads the whole conversation.
...
Each subsequent `tool` record has a consecutive `seq` starting at 1, a unique `ref` (optional; defaults to `s<seq>`), an allowed `tool` and `args`. The browser supplies the pinned `clipId`. Earlier scalar tool results can be referenced with `{"$ref":"alias","field":"effectId"}` or `nodeId`. `{"op":"end","lastSeq":N}` and a closing fence terminate the stream. Partial lines never execute, and ordinary prose/code fences do not trigger graph changes.

Supported operations are visual-effect add/update/remove, effect keyframes (`addKeyframe { effectId, param, keys: [{ time, value, easing? }] }` animates one effect parameter or exposed `<nodeId>_value` as one undoable sequence; `effectId` may reference an effect created earlier in the block), Flock node add/update/remove/connect/disconnect, `createImageNodeGraph` and `editOperatorGraph`, using deterministic handlers, policy, audit and undo boundaries. Color and parameter-source graph authoring are not yet supported by this stream. The Fast/kernel route still uses its normal operation plans. Malformed or disallowed operation records and failed steps are skipped: later records still run, and each skipped step is reported to the model with its next tool result. A missing `ref` defaults to `s<seq>`, and `seq`/`lastSeq` written as numeric strings are accepted. Lost clip ownership or cancellation stops later work; completed steps remain visible and undoable. A reload does not replay streamed mutations. Limits are 128 operations, 64 KiB per line and 1 MiB per block.
...
Node IDs start with a letter and use only letters, digits, `_` and `-`. Dots are mapped to `-` in `editOperatorGraph` and `getOperatorGraph` arguments (for example `key.split` becomes `key-split`), consistently for adds and later references, and the result lists the mapping in `renamedNodeIds`. Invalid and duplicate IDs are rejected with separate messages. Compound nodes (registered compositions) accept a caller-chosen `nodeId` like any other node. They are expanded while editing, so `connect` resolves the compound ID, or the `@compound-<id>` handle an automatic `add` returns, together with a public port ID from `getNodeDefinitions` to the matching inner endpoints; an unknown public port is rejected with the available port list. Image-graph signal types stay strict (alpha is not number, rgb is not vec3). When a connection mismatches and exactly one parameterless conversion node exists, such as the identity `convert.alpha-to-scalar` or `convert.scalar-to-alpha`, `connect` inserts it between the ports in the same undo step and reports `insertedConversion`; otherwise the error names the available conversions. `remove` with a compound ID deletes the whole expanded compound (all members, nested groups and cables).

Inside a stream block, `editOperatorGraph` records may omit `effectId`; it defaults to the graph the block last created or named explicitly. When several same-typed outputs fit an input (for example `value` and `weight`), an omitted source port uses the first declared output. The agent builds graphs linearly in dataflow order: one `add` per node carries its wiring and values, and references only nodes that already exist. `inputs` is either `{ targetPort: "node" | "node.port" }` or an ordered list of sources that fill the first free compatible inputs; `params` sets parameter values; `min`/`max`/`step` (with `label`) configure a Value slider. A source port may be omitted when the source has one output or exactly one output fits the target, and `connect` accepts the same inference (a missing `toPortId` takes the first free compatible input). Without `position`, a wired node is placed right of its sources. A composite add is one undo step and leaves the graph unchanged if any part fails; its result lists the `connected` cables and the new node's `openInputs`. Separate `connect` records remain for rewiring, feedback and the final output cable.

Streamed records execute from text deltas, so the model does not receive their individual results. Failed stream steps are therefore attached, once, to the model's next editor tool result, with the step number, target and error, and an instruction to inspect and repair the affected graph. If the turn ends with failed stream steps and an edited graph still paused as incomplete (or failures the model never saw), the chat answer gets a visible warning naming the incomplete effect and its reason. Steps rejected before execution (malformed records, missing or unknown effect arguments) also appear as failed calls in the chat history and activity log, and their error names the expected argument, e.g. `addEffect needs effectType`.
...
| `geometry.face` | Landmarks + topology → UV geometry | `landmarksToMesh`; topology supplied by the face adapter |
...
| `geometry.depth` | Calibrated depth → UV geometry | `depthToMesh` |
| `geometry.merge-surface` | Primary mesh + depth mesh → geometry | `mergeSurfaceMeshes`; outline cut, clipped exterior, seam blend and subdivision |
| `collision.mesh` | Geometry → collider | `meshCollision`; indexed mesh, independent of MediaPipe |
| `forces.wind` | Direction, strength, gust + optional scalar → force | `wind.ts`; shared with Flock |
...
`compileSceneGraph` follows connected
