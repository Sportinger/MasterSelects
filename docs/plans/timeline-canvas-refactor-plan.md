...
**Status:** Phase 0 contracts implemented; Phase 1 shell-mount/diagnostics implemented behind the legacy overlay; Phase 2 cache-boundary and Phase 4 legacy-retirement slices in progress, current checkpoint tested with TypeScript, lint, Vitest, and production build
...
**Latest implementation checkpoint:** six Phase 0 contract packets landed through the swarm. `ClipInteractionShell` now mounts inside the existing canvas DOM overlay for hovered/dragged/trimmed canvas clips plus track-local active `clipFade`, open clip context-menu, selected keyframe, audio-region, spectral-region, video-bake, and stem-job states with `pointer-events: none`, while selected-only clips still stay canvas-rendered. Active trim handles now render through `ClipTrimHandles` and dispatch back into the existing trim command path; active fade shells now render `ClipFadeHandles`, share fade-curve geometry with `TimelineClipCanvas`, use duration-positioned fade handles, and dispatch into the existing fade command path. Selected keyframe tick shells now render `ClipKeyframeTicks` through the shell and dispatch grouped keyframe moves through `TimelineTrack`. Audio-region shells now render selection/gain visuals, operation overlays, edit-stack controls, and context-menu commands through `ClipAudioRegionControls`, and own move, resize, gain, fade-in, fade-out, preview, and commit interactions through store actions. Spectral-region shells now render selection and image-layer overlays, Mask/Resyn/Img toolbar commands, Ctrl/Meta drag selection, selection clear, and image-layer drop through `ClipSpectralRegionControls`, with real `ClipSpectrogram` retirement still deferred until canvas spectrogram parity exists. Video-bake shells now render clip bake-region overlays and Bake/Unbake/Remove controls through `ClipVideoBakeControls`, reusing the existing region overlay model and store actions. Stem-only shells now render active job progress and completed-stem switcher controls through `ClipStemControls`, including source/stem prewarm and `setClipSourceToStem`. Trim-only, hover-only, context-menu-only, context-menu-plus-hover, drag/multi-drag-only, drag/multi-drag-plus-hover, stem-only, fade-only, keyframe-only, audio-region-only, spectral-region-only, video-bake-only, and mixed parity-shell active canvas states now skip the legacy `TimelineClip` body entirely, and diagnostics count actual legacy clip bodies instead of all mounted shells. `getStats` now exposes `timelineCanvas` diagnostics with canonical store track/clip totals, active renderer totals, last reported renderer totals, stale track counts, missing track IDs, canvas draw counts, thumbnail/waveform counts, DOM overlay counts, shell counts, and active shell slot counts. Phase 2 has moved visible thumbnail DB warmup into `timelineThumbnailDbWarmup`, missing visible thumbnail generation into `timelineThumbnailGenerationWarmup`, waveform artifact reads into `timelineWaveformArtifactWarmup`, and source waveform generation demand into `timelineSourceWaveformWarmup`, all with in-flight coalescing and component-side visible-demand collection. Thumbnail and waveform generation warmups block while playback or drag preview is active. Missing visible thumbnail generation now resolves from `MediaFile.url` before falling back to hydrated preview video URLs, runs through `thumbnailCacheService.generateForSourceUrl()`, and uses a small global generation cap. Warmup schedulers now use bound browser timers so visible thumbnail generation cannot crash with `Illegal invocation` on load. Thumbnail cache notifications now include frame/second-index events; `TimelineClipCanvas` and the DOM thumbnail hook filter redraws to visible source-second ranges. Decoded thumbnail `ImageBitmap`s now live in a service-level `thumbnailBitmapCache` with source/URL close APIs; source thumbnail evictions close decoded bitmaps before revoking blob URLs. `TimelineClip.tsx` decomposition now includes extracted presentation, stem-display, audio-region display, coverage badges, media-label color resolution, waveform render geometry, audio-region context-menu model, active audio/video region overlay geometry, source-extension ghost geometry, and spectral selection/image-layer overlay geometry. Suppressed canvas overlay bodies no longer render passive labels, badges, thumbnails, transcript/analysis markers, reload indicators, or mixdown/static-artwork decorations while active handles stay available.

**Latest retirement checkpoint:** `TimelineClip.tsx` still mounts as the legacy canvas overlay for active states that do not yet have shell parity, but trim-only, hover-only, context-menu-only, context-menu-plus-hover, drag/multi-drag-only, drag/multi-drag-plus-hover, stem-only, fade-only, keyframe-only, audio-region-only, spectral-region-only, video-bake-only, mixed parity-shell active canvas states, and context-menu-plus-parity-module states no longer call `renderClip()` and therefore no longer create the legacy clip body. Remaining canvas-mode overlay instances receive `passiveVisualsSuppressed`, which prevents thumbnail, waveform, spectrogram, audio-artifact warmup/render work, and passive decoration JSX from running through the invisible overlay. `serializationUtils.restoreSourceThumbnails()` no longer calls `thumbnailCacheService.generateForSource()` during timeline load; it only loads already-cached source thumbnails, leaving missing thumbnail generation to the visible scheduler. Silent project-load auto-relink now passes `{ generateThumbnails: false }` through both `applyRelinkMatch()` and the stored-handle `updateTimelineClips()` fallback, while manual reload/relink keeps direct generation. Unused IndexedDB project actions (`saveProject`, `loadProject`, `getProjectList`, `deleteProject`) were removed from `mediaStore/slices/projectSlice.ts`; current project loading/saving stays on `ProjectFileService`. The remaining IndexedDB startup restore body was extracted to `mediaStore/legacyStartupRestore.ts`, leaving `projectSlice.ts` as a thin compatibility delegator plus project UI/runtime actions.

**Latest decomposition checkpoint:** Post-commit `TimelineClip.tsx` was reduced from 3364 LOC to 860 LOC while keeping the legacy overlay behavior available. New focused renderers/hooks now own thumbnail filmstrips, media classification, audio-region context menus, video-bake region overlays, audio-edit operation overlays, clip overlay action callbacks, clip metadata, coverage badges, passive status badges, stem switcher UI/state, clip animation state, keyframe tick drag state, spectral image-layer selection/drop handling, audio artifact warmup scheduling, audio analysis display state, audio-display derived state, audio render-state planning, timeline render geometry/thumbnail windows, timeline tool pointer dispatch/cut indicators, visual chrome/source-extension ghosts, active region overlay state, timeline/media-store binding selection, audio waveform/spectrogram media view, audio-media prop planning, passive background/foreground visual layers, audio-region selection overlay, audio-region gain/context-menu controls, audio/video-bake/spectral region interaction drag controllers, spectral-region overlays, audio edit-stack controls, transcript/analysis passive overlays, keyframe tick rendering, fade/trim controls, and selection hitareas. `TimelineClipProps` no longer carries unused `clipFade`, `timelineRef`, `pixelToTime`, `allKeyframeTimes`, or clip pick-whip pass-through props; unused `TimelineTrackProps.timelineRef`, dead audio gain-drag snapshot fields, unused waveform/spectrogram wrapper hooks, and the dead timeline thumbnail-bitmap re-export were also removed. Stem job active-phase handling, clip source timing/loop-extension helpers, trim minimum duration, timeline render overscan, and empty stem choices are now shared instead of duplicated in UI, store, trim, and canvas files. Thumbnail render-plan work is now gated behind visibility flags and `ClipThumbnailFilmstrip` uses discriminated props so invalid blank filmstrip combinations fail type checking. `useClipAudioRegionControls.ts` now destructures stable local bindings for callback dependencies, removing the remaining React hook lint warnings. Current checkpoint passed `npx tsc -p tsconfig.app.json --noEmit --pretty false`, `npm run lint`, `npm run test` (325 files, 3448 tests), and `npm run build`.
...
- Current retirement candidates:
  - Full `TimelineClip` mounting inside `.timeline-canvas-dom-overlay` once shell modules cover active controls.
    - Done for trim-only, hover-only, context-menu-only, context-menu-plus-hover, drag/multi-drag-only, drag/multi-drag-plus-hover, stem-only, fade-only, keyframe-only, audio-region-only, spectral-region-only, video-bake-only, mixed parity-shell active states, and context-menu-plus-parity-module states.
...
- Move only proven active controls out of `TimelineClip`.
  ...
  - Done for spectral-region selection/image-layer overlays, toolbar commands, Ctrl/Meta drag selection, clear, and image-layer drop through `ClipSpectralRegionControls`; spectral-region-only shells now skip the legacy body, but DOM `ClipSpectrogram` retirement remains deferred until canvas spectrogram parity exists.
...
- Add cache scheduler and in-flight artifact load coalescing.
  ...
  - Pending for non-waveform audio artifact reads.
...
- Extract audio, spectral, video-bake, and stem modules.
  ...
  - Done for the spectral shell module: active selection overlays, image-layer overlays/drop, and toolbar controls are in shell; DOM spectrogram retirement is still deferred until real canvas spectrogram parity.
  - Next recommended parallel slice from the read-only agent pass: renderClip contract retirement and full overlay removal audit, with DOM spectrogram retirement still deferred.
