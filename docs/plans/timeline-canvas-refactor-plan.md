...
- Current retirement candidates:
  - Full `TimelineClip` mounting inside `.timeline-canvas-dom-overlay` once shell modules cover active controls.
    - Done for trim-only, hover-only, context-menu-only, context-menu-plus-hover, drag/multi-drag-only, drag/multi-drag-plus-hover, stem-only, fade-only, and keyframe-only shells; audio/spectral/video-bake still need shell parity before deletion.
...
- Move only proven active controls out of `TimelineClip`.
  ...
  - Done for selected keyframe clip-bar ticks: `ClipKeyframeTicks` now renders through `ClipInteractionShell`, routes grouped keyframe moves through `TimelineTrack`, and skips the legacy body for keyframe-only shells while mixed module shells still keep the fallback.
...
- Extract audio, spectral, video-bake, and stem modules.
  ...
  - Next recommended parallel slices from the read-only agent pass: video-bake after hit-area parity tests, spectral extraction only with DOM spectrogram retirement deferred until real canvas spectrogram parity.
