...
**Latest decomposition checkpoint:** Post-commit `TimelineClip.tsx` was reduced from 3364 LOC to 2624 LOC while keeping the legacy overlay behavior available. New focused renderers now own thumbnail filmstrips, media classification, audio-region context menus, video-bake region overlays, audio-edit operation overlays, clip metadata, coverage badges, passive status badges, stem switcher UI, audio waveform/spectrogram media view, audio-media prop planning, passive background/foreground visual layers, audio-region selection overlay, spectral-region overlays, audio edit-stack controls, transcript/analysis passive overlays, keyframe tick rendering, fade/trim controls, and selection hitareas. `TimelineClipProps` no longer carries unused `clipFade`, `timelineRef`, `pixelToTime`, or clip pick-whip pass-through props; unused `TimelineTrackProps.timelineRef` and the dead timeline thumbnail-bitmap re-export were also removed. Thumbnail render-plan work is now gated behind visibility flags and `ClipThumbnailFilmstrip` uses discriminated props so invalid blank filmstrip combinations fail type checking. No playback/browser test has been run for this checkpoint; only `tsc --noEmit` has been used, per the current no-test instruction.
