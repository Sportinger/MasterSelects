...
- Current retirement candidates:
  - Full `TimelineClip` mounting inside `.timeline-canvas-dom-overlay` once shell modules cover active controls.
    - Done for trim-only, hover-only, context-menu-only, context-menu-plus-hover, drag/multi-drag-only, and drag/multi-drag-plus-hover shells; fade/keyframe/audio/spectral/video-bake/stem still need shell parity before deletion.
...
- Extract audio, spectral, video-bake, and stem modules.
  - Next recommended parallel slices from the read-only agent pass: fade shell handles first without deleting the legacy fade body; stem shell module as the most disjoint active-control extraction; audio-region shell module as the next larger parity slice; video-bake after hit-area parity tests; spectral extraction only, with DOM spectrogram retirement deferred until real canvas spectrogram parity.
