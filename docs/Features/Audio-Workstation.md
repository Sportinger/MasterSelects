...
- Dock layout transitions run an idle snapshot/measure/WAAPI warmup after the first render and animate Timeline, Preview, and Media panels live instead of deep-cloning their contents, preventing image/video-heavy Board views from blocking switches between `VIDEO EDIT` and `AUDIO EDIT`.
