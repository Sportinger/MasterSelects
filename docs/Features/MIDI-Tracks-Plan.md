...
- **In-clip note preview** (`components/ClipMidiPreview.tsx`): a Cubase-style mini note view drawn on a canvas — one thin bar per note from note-on to note-off. **X is clip-local time, mapped 1:1** (`x = note.start × zoom`), so the preview spans the full clip width and lines up exactly with the timeline ruler (the canvas spans the whole clip; only the backing store is capped for very wide clips). **Y is pitch**, fit to the clip's own min..max range (DAW "fit notes to view") so the used register fills the height. A note whose tail crosses the clip end is clamped at the boundary, and a note starting past the end is not drawn — mirroring the playback scheduler exactly (the clip is a *window* onto the notes: content past `clip.duration` is silent). Only renders when the clip has notes.
- **Clip length rules the editor**: the piano roll's content width equals the clip's real duration (`contentWidth = clipDuration × PX_PER_SEC`, `PianoRoll.tsx`) — no minimum padding — so the editor and the clip preview represent the same `[0, duration]` span. Composing more means resizing the clip on the timeline.
