...
- In camera Edit mode, the selected scene clip automatically becomes the independent editor camera's orbit pivot, including camera clips. Changing the selection blends to the new pivot over 200 ms; clearing the selection returns the pivot to scene origin `(0, 0, 0)`. Double-click and the object's right-click `Set Orbit Pivot` action remain available for explicit focusing.
